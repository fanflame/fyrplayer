...
class TowVBOManager : IDrawer() {
    ...
    private fun config2() {
        ...
        val buffer = ByteBuffer.allocateDirect(vertexCoord1.size * 4).order(ByteOrder.nativeOrder()).asFloatBuffer()
        buffer.put(vertexCoord1).position(0)
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, vertexCoord1.size * 4, buffer, GLES30.GL_STATIC_DRAW)
    ...
    private fun config1() {
        ...
        GLES30.glGenBuffers(1, vbo, 0)
        GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, vbo[0])
        val buffer = ByteBuffer.allocateDirect(vertexCoord2.size * 4).order(ByteOrder.nativeOrder()).asFloatBuffer()
        buffer.put(vertexCoord2).position(0)
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, vertexCoord2.size * 4, buffer, GLES30.GL_STATIC_DRAW)
        GLES30.glVertexAttribPointer(0, 3, GLES30.GL_FLOAT, false, 4 * 3, 0)
        GLES30.glEnableVertexAttribArray(0)
        OpenglProgramHelper.checkError()
    }
