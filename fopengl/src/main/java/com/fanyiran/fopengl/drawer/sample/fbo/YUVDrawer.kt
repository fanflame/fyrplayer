...
//    }
    ...
    fun updateYUV(byteBuffer: ByteArray, width: Int, height: Int) {
        if (lastWidth != width || lastHeight != height) {
            lastWidth = width
            lastHeight = height
            yData = ByteBuffer.allocateDirect(width * height)
            uvData = ByteBuffer.allocateDirect(width * height / 2)
        }
    ...
    override fun config() {
        ...
        val vertexBuffer = ByteBuffer.allocateDirect(pointAttributeData().size * FLOAT_SIZE)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer()
                .put(pointAttributeData())
                .position(0)
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, pointAttributeData().size * FLOAT_SIZE, vertexBuffer, GLES30.GL_STATIC_DRAW)
